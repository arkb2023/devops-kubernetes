...
**Application Updates**
- Enhanced `Log Output` application [deployment.yaml](./manifests/deployment.yaml) resource to mount PVC into `Log output` container 
- Enhanced `Ping Pong` application [deployment.yaml](../ping-pong/manifests/deployment.yaml) resource to mount the same PVC into `Ping pong` container 
- Added `Ping Pong` application [ingress](../ping-pong/manifests/ingress.yaml) resource to route requests with path prefix `/pingpong` to the `ping-pong-svc` service on port `3456`.
